...
void Conveyor::run_parallel(size_t cars_cnt)
{
    ...
    {
        std::shared_ptr<SlauSolver> new_car(new SlauSolver);
...
void Conveyor::run_linear(size_t cars_cnt)
{
    ...
    for (size_t i = 0; i < cars_cnt; i++)
    {
        std::shared_ptr<SlauSolver> car = q1.front();
...
void Conveyor::create_engine()
{
    ...
    do
    {
        if (!this->q2.empty())
        {
            std::shared_ptr<SlauSolver> car = q2.front();
...
void Conveyor::create_wheels()
{
    ...
    do
    {
        if (!this->q3.empty())
        {
            std::shared_ptr<SlauSolver> car = q3.front();
